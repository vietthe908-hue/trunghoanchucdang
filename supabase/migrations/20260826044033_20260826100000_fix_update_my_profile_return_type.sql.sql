/*
# Fix update_my_profile return type

PostgREST cannot auto-resolve generic "record" return types. Switch to
SETOF with a concrete return type using the profiles table row type.
*/

-- Drop the old function (signature with TEXT params)
DROP FUNCTION IF EXISTS public.update_my_profile(text, text, text, text);

-- Recreate with SETOF public.profiles return type
